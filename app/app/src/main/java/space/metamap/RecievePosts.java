...
class RecievePost {
    ...
    public static void receivePost(final Context context, final double latitude, final double longitude, final PostList postList) {
        ...
        try {
            Request jsonRequest = new Request(Request.Method.POST, url, new JSONObject(String.format("{\"latitude\": \"%f\", \"longitude\":\"%f\"}", latitude, longitude)), new Response.Listener<JSONObject>() {
                @Override
                public void onResponse(JSONObject response) {
                    try {
                        ...
                        for (int i=0; i<data.length(); i++) {
                            ...
                            postList.addToList(new Post(item.get("data"), (String) item.get("username"), coordinate));
                        }
